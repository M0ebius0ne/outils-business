...
def generate_article_idea(existing_articles):
    prompt = f"""
Tu es un expert en logiciels SaaS B2B, en outils marketing et en stratégie SEO "Longue Traîne".
Je veux écrire un article de blog SEO qui présente un logiciel métier très utile (CRM, Emailing, Création de site, Productivité, Design) mais DE NICHE ou TRÈS RÉCENT.
L'outil doit être réel, mais ne choisis SURTOUT PAS les leaders absolus du marché que tout le monde connaît comme Salesforce, Shopify, WordPress, ou Mailchimp.
Trouve un outil très spécifique (ex: un outil d'emailing dédié aux podcasteurs, un CRM minimaliste pour freelances, une plateforme de webinars innovante, etc) qui a probablement un programme d'affiliation public.
...
- "slug": un identifiant url-friendly (ex: "nom-outil-crm-freelance")
- "url": l'url officielle de l'outil
- "description": une brève description de ce que fait l'outil
- "long_tail_keyword": un mot clé de longue traîne très spécifique pour lequel on veut ranker (ex: "Meilleur CRM minimaliste pour les graphistes freelances")
"""
...
Tags: business, saas, marketing, {tool_data['slug']}
Slug: {tool_data['slug']}
Author: IA
